...
#         nargs="*",
#         default=None,
#         help="Optional list of Task IDs to filter the benchmark rows. "
#              "If omitted, run on ALL tasks."
#     )
#     parser.add_argument(
#         "--ns",
#         type=int,
...
#!/usr/bin/env python3
"""
Generate questions based on content from Ground Truth (GT), 
Discovera or LLM-generated reports.
Each run is saved into a timestamped folder. Logs and outputs are stored
inside that folder to keep runs organized and avoid overwriting results.
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional experiment ID. If not provided, a timestamp-based ID will be generated."
    )
    return parser.parse_args()
...
def main():
    ...
    if args.experiment_id:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        experiment_id = f"{args.model}_{timestamp}"

    experiment_dir = os.path.join(args.out, f"{experiment_id}/questions")
    os.makedirs(experiment_dir, exist_ok=True)
    ...
    for col in args.report_columns:
        ...
        # --- Filter reports by task_ids if provided ---
        if args.task_ids is not None:
            reports = [r for r in reports if r[0] in args.task_ids]
            logging.info(f"Filtered reports to {len(reports)} tasks based on --task-ids")

        if not reports:
            logging.warning(f"No reports found for column {col} with the given task IDs.")
            continue
